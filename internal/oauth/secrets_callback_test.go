...
// A caller that fails to supply an expected state must not accidentally disable validation.
func TestSecretsCallbackHandler_emptyExpectedState(t *testing.T) {
	log := zerolog.Nop()
	codeCh := make(chan string, 1)
	h := SecretsCallbackHandler(codeCh, "", &log)

	req := httptest.NewRequest(http.MethodGet, "/callback?code=c&state=anything", nil)
	rr := httptest.NewRecorder()
	h(rr, req)
