...
func RunSecretsHappyPath(t *testing.T, tc TestConfig, chainName string, capRegAddress string) {
	t.Helper()
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/graphql" {
			...
			if testutil.QueryIsGetTenantConfig(req.Query) {
				_ = json.NewEncoder(w).Encode(testutil.MockGetTenantConfigGraphQLPayloadWithCapReg(capRegAddress))
				return
			}
