...
type newIdlTypeDefinedNamed struct {
	Name string `json:"name"`
}

type newIdlTypeDefined struct {
	Defined newIdlTypeDefinedNamed `json:"defined"`
}
...
func (env *IdlType) UnmarshalJSON(data []byte) error {
	...
	case map[string]interface{}:
		...
		if _, ok := v["defined"]; ok {
			...
			if err := utilz.TranscodeJSON(temp, &target); err != nil {
				var target2 newIdlTypeDefined
				if err2 := utilz.TranscodeJSON(temp, &target2); err2 != nil {
					return err
				}
				target = IdlTypeDefined{Defined: target2.Defined.Name}
			}
			typeFound = true
