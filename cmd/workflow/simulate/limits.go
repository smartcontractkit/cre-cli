...
// AptosChainWriteReportSizeLimit returns the Aptos chain write report size limit in bytes.
func (l *SimulationLimits) AptosChainWriteReportSizeLimit() int {
...
func (l *SimulationLimits) LimitsSummary() string {
	w := &l.Workflows
	return fmt.Sprintf(
		"HTTP: req=%s resp=%s timeout=%s | ConfHTTP: req=%s resp=%s timeout=%s | Consensus obs=%s | ChainWrite evm_report=%s evm_gas=%d aptos_report=%s aptos_gas=%d solana_report=%s solana_cu=%d | WASM binary=%s compressed=%s",
