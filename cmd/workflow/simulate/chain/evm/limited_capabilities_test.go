...
type stubEVMChainLimits struct {
	reportSizeLimit int
	gasLimit        uint64
}

func (s *stubEVMChainLimits) ChainWriteReportSizeLimit() int { return s.reportSizeLimit }
func (s *stubEVMChainLimits) ChainWriteEVMGasLimit() uint64  { return s.gasLimit }
...
func TestLimitedEVMChainWriteReportRejectsOversizedReport(t *testing.T) {
	...
	limits := &stubEVMChainLimits{reportSizeLimit: 4}
...
func TestLimitedEVMChainWriteReportRejectsOversizedGasLimit(t *testing.T) {
	...
	limits := &stubEVMChainLimits{gasLimit: 10}
	inner := &evmClientCapabilityStub{}
	wrapper := NewLimitedEVMChain(inner, limits)
...
func TestLimitedEVMChainWriteReportDelegatesOnBoundaryValues(t *testing.T) {
	...
	limits := &stubEVMChainLimits{reportSizeLimit: 4, gasLimit: 10}
