...
func (f fakeUserDonLimitClient) CheckUserDonLimit(_ context.Context, owner common.Address, donFamily string, pending uint32) error {
...
func (f fakeUserDonLimitClient) GetWorkflowListByOwnerAndName(_ context.Context, _ common.Address, _ string, _, _ *big.Int) ([]workflow_registry_v2_wrapper.WorkflowRegistryWorkflowMetadataView, error) {
...
func TestCheckUserDonLimitBeforeDeploy(t *testing.T) {
	...
	t.Run("errors when limit reached", func(t *testing.T) {
		...
		err := checkUserDonLimitBeforeDeploy(context.Background(), client, nameLookup, owner, donFamily, workflowName, true, nil)
		require.Error(t, err)
	...
	t.Run("accounts for keepAlive false pausing same-name workflows", func(t *testing.T) {
		...
		err := checkUserDonLimitBeforeDeploy(context.Background(), client, nameLookup, owner, donFamily, workflowName, false, nil)
		require.NoError(t, err)
	})
	...
	t.Run("skips check when updating existing workflow", func(t *testing.T) {
		...
		err := checkUserDonLimitBeforeDeploy(context.Background(), client, nameLookup, owner, donFamily, workflowName, true, &existingStatus)
		require.NoError(t, err)
	})
}
